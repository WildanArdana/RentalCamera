...
<?php
$kuericart = mysqli_query($koneksi, "SELECT p.`nama` AS namaproduk, p.`gambar` AS gambar, p.`stok` as stok, o.`jumlah` AS jumlah, o.`id_produk` as idproduk, o.`harga` AS harga, m.nama AS namamerek FROM `tbl_produk` p,`tbl_order` o,`tbl_merek` m WHERE o.`id_session` = '$sessionid' AND p.`id_produk` = o.`id_produk`");
$i = 1;
$total = 0;
